...
const buyOrder = computed(() => {
  return buyPrice.value && amount.value && feeBuy.value
    ? (buyPrice.value * amount.value * (1 + feeBuy.value / 100)).toFixed(2)
    : null; // Replace '1' with real amount
});
const stopLoss = computed(() => {
  if (buyPrice.value && slPrice.value && amount.value && feeSell.value) {
    return (
      slPrice.value * amount.value * (1 + feeSell.value / 100) -
      buyOrder.value
    ).toFixed(2);
  }
  return null;
...
const takeProfit = computed(() => {
  if (buyPrice.value && tpPrice.value && amount.value && feeSell.value) {
    return (
      tpPrice.value * amount.value * (1 + feeSell.value / 100) -
      buyOrder.value
    ).toFixed(2);
  }
  return null;
...
const t_ZeroPrice = computed(() => {
  if (buyPrice.value && t_TpIndex.value && digits.value) {
    return (buyPrice.value * (1 + parseFloat(t_TpIndex.value))).toFixed(
      digits.value
    );
  }
  return null;
  ...
  <div class="border-b py-2">
    <div class="flex justify-between items-center pb-2">
      ...
      <button
        ...
        class="flex px-2 font-bold text-red-600 border border-red-600 items-center"
      >
        x
      </button>
    </div>
