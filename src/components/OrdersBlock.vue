...
const { removeBlock, addOrderToBlock, updateBlockSymbol } = useOrdersBlockStore();
</script>
	...
	<div class="max-h-24 overflow-y-auto text-sm pt-2 border-b border-green-600">
		<div v-for="(block, index) in blocks" :key="block.id" :id="'block-' + block.id" class="">
			<div class="flex justify-between pb-1 border-b">
				<input :id="'symbol-' + block.id" type="text" :value="block.symbol"
					@input="updateBlockSymbol(block.id, $event.target.value)" placeholder="Symbol"
				...
				<button @click="addOrderToBlock(block.id)" class="px-2 font-bold text-green-600 border border-green-600">
					+
				</button>
			</div>
